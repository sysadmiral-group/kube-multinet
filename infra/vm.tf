...
resource google_compute_instance node_mn_0 {
  name         = "kube-mn-n2-0"
  machine_type = "n2-standard-4"
  project = local.gcp_project
  zone         = local.zone
  ...
  metadata_startup_script = <<-EOF
    echo "1 rt1" | sudo tee -a /etc/iproute2/rt_tables
    ip route add 192.168.1.1 src 192.168.1.10 dev eth1 table rt1
    ip route add default via 192.168.1.1 dev eth1 table rt1
    ip rule add from 192.168.1.10/32 table rt1
    ip rule add to 192.168.1.10/32 table rt1

    echo "2 rt2" | sudo tee -a /etc/iproute2/rt_tables
    ip route add 10.44.1.1 src 10.44.1.10 dev eth2 table rt2
    ip route add default via 10.44.1.1 dev eth2 table rt2
    ip rule add from 10.44.1.10/32 table rt2
    ip rule add to 10.44.1.10/32 table rt2
  EOF
